...
export const testEmail = async (req, res, next) => {
  try {
    ...
    const sendEmail = await mailer.sendTransacEmail({
      sender,
      to: receiver,
      // subject: "how are you",
      ...
      params: {
        total: 4000,
        items: [
          { name: "Cashew", price: 1000, qty: 2 },
          { name: "Badam", price: 500, qty: 1 },
        ],
      },
    });
    return successResponse(req, res, { sendEmail });
  } catch (error) {
    return next(error);
  }
};

export const aliveTest = async (req, res, next) => {
  try {
  } catch (error) {
    return next(error);
  }
